    ...
    <div className="max-h-full min-h-screen w-full font-[Gilroy-Semibold] bg-slate-300 text-black dark:bg-zinc-900 dark:text-white">
      ...
      <form action="/" className='lg:w-[55%] mg:w-[70%] sm:w-[90%] p-9 m-auto'>
        ...
        <div className="bg-zinc-800 p-7 rounded-lg border-t-[10px] border-t-sky-600 mb-10">
          <div className={`md:text-3xl text-2xl bg-transparent text-blue-400 ${quiz.title.isBold ? "font-[Gilroy-Semibold]": "font-[Gilroy-Regular]"} ${quiz.title.isUnderline? "underline": ""} ${quiz.title.isItalic ? "italic" : ""}`}>
            {quiz.title.name}
          </div>    
        ...
        {quiz.quizes.map((item, idx)=>{
          console.log(item)
          return(
            <div key={idx} className="bg-zinc-800 p-7 rounded-lg mb-5 font-[Gilroy-Regular]">
              <div className="md:text-2xl text-lg text-violet-400">Q.{idx+1} {item.Q}</div>
              {item.options.map((option, i)=>(
                <div key={i} className="mt-2">
                  <input type="radio" className='mr-2' name={item.id} /><span className='text-sm md:text-base'>{option}</span>
                </div>
              ))}
